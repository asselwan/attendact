...
class HeuristicScorer:
    ...
    def score(self, features: dict) -> float:
        ...
        raw += FEATURE_WEIGHTS["prior_noshow_rate_12mo"] * features.get(
        ...
        raw += FEATURE_WEIGHTS["lead_time_days"] * (lead_days / 60)

        # Binary flags
        if features.get("is_ramadan"):
            raw += FEATURE_WEIGHTS["is_ramadan"]
        if features.get("is_eid_window"):
            raw += FEATURE_WEIGHTS["is_eid_window"]
        if features.get("is_summer_exodus"):
            raw += FEATURE_WEIGHTS["is_summer_exodus"]
        if features.get("is_post_iftar"):
            raw += FEATURE_WEIGHTS["is_post_iftar"]

        # Distance band
        distance_scores = {"<5km": 0.0, "5-15km": 0.3, "15-30km": 0.6, "30km+": 1.0, "unknown": 0.3}
        raw += FEATURE_WEIGHTS["distance_band"] * distance_scores.get(
            features.get("distance_band", "unknown"), 0.3
        )

        # Heat index band
        heat_scores = {"low": 0.0, "moderate": 0.3, "high": 1.0}
        raw += FEATURE_WEIGHTS["heat_index_band"] * heat_scores.get(
        ...
        raw += FEATURE_WEIGHTS["insurance_tier"] * tier_scores.get(
        ...
        raw += FEATURE_WEIGHTS["booking_channel"] * channel_scores.get(
        ...
        raw += FEATURE_WEIGHTS["time_of_day_band"] * tod_scores.get(
            features.get("time_of_day_band", "morning"), 0.4
        )

        # Specialty baseline rate (continuous, 0-1)
        raw += FEATURE_WEIGHTS["specialty_baseline_rate"] * features.get(
            "specialty_baseline_rate", 0.21
        )

        # Age band
        age_scores = {"0-17": 0.5, "18-39": 0.6, "40-64": 0.3, "65+": 0.2}
        raw += FEATURE_WEIGHTS["patient_age_band"] * age_scores.get(
